...
import org.firedetection.biz.board.dao.CsBoardDAO;
import org.firedetection.biz.board.vo.CsBoardVO;
...
public class CsBoardServiceImpl implements CsBoardService{
	...
	public HashMap<String, Object> viewBoard(int cs_num) {
		cs_dao.countUp(cs_num);
		HashMap<String, Object> result = new HashMap<String, Object>();
		CsBoardVO vo = cs_dao.viewBoard(cs_num);
		result.put("board", vo);
		result.put("prevfb_num", cs_dao.prevfb_num(cs_num));
		result.put("nextfb_num", cs_dao.nextfb_num(cs_num));
		return result;
	}
