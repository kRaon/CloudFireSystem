...
import org.firedetection.biz.board.dao.FileDAO;
import org.firedetection.biz.board.dao.QnaBoardDAO;
import org.firedetection.biz.board.dao.ReplyDAO;
...
public class QnaBoardServiceImpl implements QnaBoardService {
	...
	public int deleteQnaBoard(int fb_num) {
		...
		row += fb_dao.deleteQnaBoard(fb_num);
		return row;
	}
	...
	public HashMap<String, Object> boardList(int page_num) {
		...
		// 결과값을 list에 넣는다.
		List<QnaBoardVO> list = fb_dao.boardList(map);
	...
	/**
	 * @param page_num
	 *            검색된 게시판 목록에서 현재 보고있는 페이지 게시판 목록을 반환하는 함수
	 */
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public HashMap<String, Object> scBoardList(String condition, String search, int page_num) {
		// 전체 게시판 개수를 가져옴
		int total_boards = fb_dao.scCountBoards(condition, search);
		// 게시판을 10개씩 페이징 처리했을 때, 총 몇개의 목록이 나오는지 계산
		int count_page = (total_boards + 9) / 10;
		// 만약 마지막 페이지의 게시글이 1개 인데 삭제된 경우
		if (count_page < page_num)
		...
		// 결과값을 list에 넣는다.
		List<QnaBoardVO> list = fb_dao.scBoardList(map);
		// 총 페이지수가 몇개인지도 보내줘야 하므로 Map구조로 put해서 result를 return
		result.put("list", list);
		result.put("count_page", count_page);
		result.put("thisPage", page_num);
	...
	public HashMap<String, Object> viewBoard(int fb_num) {
		...
		HashMap<String, Object> result = new HashMap<String, Object>();
		QnaBoardVO vo = fb_dao.viewBoard(fb_num);
		result.put("board", vo);
	...
	/**
	 * @param 현재
	 *            검색된 게시판 번호를 통해 게시판의 정보를 담아 반환하는 함수
	 */
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
